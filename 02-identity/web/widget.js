// The Grand Meridian — concierge chat widget (module 02 variant).
//
// Diff vs 00/01/03/04: adds a sign-in pill above the input row that fetches a
// user token from mock_idp (POST /oauth2/login) and threads it into chat
// requests as `context.user_token`. The agent's book_room tool exchanges that
// for an agent-acting-on-user token via RFC 8693 — module 02 README, step 4.
// Without sign-in, bookings still go through but mock_booking logs
// `on_behalf_of=<none>` (the contrast moment that makes the lesson land).
//
...
(() => {
  ...
  const IDP_LOGIN_URL =
    window.GRAND_MERIDIAN_IDP_URL || "http://localhost:9700/oauth2/login";
  const GUEST_USER_ID = "user-42";

  // Identity state. Single token, single guest. No refresh, no logout — the
  // mock IdP issues 1h tokens, longer than the lab.
  let userToken = null;
  ...
  // Sign-in pill (module 02 only). Sits above the input row. Two visual states:
  // signed-out (clickable, gray) and signed-in (non-clickable, green). Anonymous
  // sending is allowed by design — see module README, "the contrast moment".
  const signInPill = document.createElement("button");
  signInPill.type = "button";
  signInPill.setAttribute("aria-label", "Sign in as a guest");
  signInPill.style.cssText = `
    margin:8px 16px 0;padding:6px 12px;border-radius:999px;cursor:pointer;
    font-family:inherit;font-size:12px;background:#F5F2E8;color:#1B2B4B;
    border:1px solid #E6E2D6;text-align:left;align-self:flex-start;
  `;
  function renderPill() {
    if (userToken) {
      signInPill.textContent = `● Signed in as Guest 42`;
      signInPill.style.background = "#EAF6EE";
      signInPill.style.borderColor = "#C2E0CB";
      signInPill.style.color = "#1F6F3E";
      signInPill.style.cursor = "default";
      signInPill.disabled = true;
    } else {
      signInPill.textContent = "Not signed in — click to sign in as Guest 42";
      signInPill.style.background = "#F5F2E8";
      signInPill.style.borderColor = "#E6E2D6";
      signInPill.style.color = "#1B2B4B";
      signInPill.style.cursor = "pointer";
      signInPill.disabled = false;
    }
  }
  renderPill();
  signInPill.addEventListener("click", signIn);
  panel.appendChild(signInPill);
  ...
  // Issue a user token by calling mock_idp directly. CORS is enabled on
  // mock_idp/main.py — without it, this fetch would fail with a misleading
  // "Failed to fetch" and the demo would die in front of the audience.
  async function signIn() {
    try {
      const res = await fetch(IDP_LOGIN_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id: GUEST_USER_ID }),
      });
      if (!res.ok) {
        showToast(`IdP rejected sign-in (${res.status}).`);
        return;
      }
      const data = await res.json();
      if (!data || !data.access_token) {
        showToast("IdP returned no access_token.");
        return;
      }
      ...
      showToast("IdP unreachable. Did you run ./run_services.sh?");
    }
  }
    ...
    try {
      const res = await fetch(ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          // When signed in, the user token rides along in context. The agent
          // exchanges it via RFC 8693 inside book_room (identity.py). When
          // signed out, context stays empty and mock_booking logs
          // on_behalf_of=<none> — the contrast that proves the user_token
          // is what populates the audit chain.
          context: userToken ? { user_token: userToken } : {},
        }),
      });
      typing.remove();
      if (!res.ok) {
